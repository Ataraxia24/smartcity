<template>
	<view>
		...
		<button type="primary" @click="onclick()">修改</button>
	</view>
</template>
		...
		methods: {
			...
			getResumeMessage() {
				...
				this.$get(api.FIND_JOB_RESUME+'1116261').then(res => {
					...
					//设置文本框中的值
					this.job_experience = res.data.experience
					this.most_edu = res.data.mostEducation
					this.address = res.data.address
					this.salary = res.data.money
					this.edu_experience = res.data.education
					this.introduce = res.data.individualResume
				})
			},
			onclick() {
				this.$put(api.MODIFY_JOB_RESUME, {
